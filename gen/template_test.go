...
func TestValuesFieldTemplate(t *testing.T) {
	type args struct {
		enum Enum
	}
	tests := []struct {
	...
	}{
		{
			...
			filepath.Join("templates", "fixtures", "string_values_field.go"),
			assert.NoError,
		},
		{
			"",
			...
			filepath.Join("templates", "fixtures", "int_values_field.go"),
			assert.NoError,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := ValuesFieldTemplate(tt.args.enum)
			tt.assertion(t, err)
